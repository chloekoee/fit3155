...
class SuffixTree:
    ...
    def __init__(self, string, encoderMode=False):
        self.encoderMode = encoderMode
        if self.encoderMode:
            self.ranks = {(item): None for item in range(len(string))}
    ...
    def get_sorted_suffixes(self, indices):
...
# #     s = file.read().replace("\n", "")
# stree = SuffixTree(s, [i for i in range(1, len(s) + 1)], encoderMode=True)
# stree.build_suffix_tree()
# print(stree.get_sorted_suffixes())
# print(stree.frequencies)
# print(stree.bwt)
