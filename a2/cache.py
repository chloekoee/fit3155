    ...
    def _check_or_insert(self, j):
        ...
        if self.aLength == 0:  ## If we are on a node
            ...
            if currentCharacter in self.aNode.edges.keys():
                ...
                self.aEdge = self.aNode.edges[currentCharacter]
                self.aLength = 1
                return False
            ...
            print(self.aNode)
            print(self.aEdge)
            self.aEdge = self.aNode.edges[self.aEdge.value]
            print(self.aEdge)
            edgeLength = self.aEdge.length(self.end)
            print(edgeLength)
            if self.aLength >= edgeLength:  ## If edgeLength <= aLength recurse deeper

                self.aNode = self.aNode.edges[self.aEdge.value].tail
                print(self.aNode)
                # print(self.aNode)
                # print(self.aNode.edges)
                # print(self.aEdge.value)
                # self.aEdge = self.aNode.edges[self.aEdge.value]
                # print(self.aNode)
                ## Calculate next character after fully matching current edge
                nextPosition = self.aEdge.start + edgeLength
                nextChar = (
                    self.text[nextPosition] if nextPosition < len(self.text) else None
                )

                self.aEdge = (
                    self.aNode.edges[nextChar]
                    if (nextChar and nextChar in self.aNode.edges)
                    else None
                )
                ...
                compareChar = self.aEdge.start + self.aLength
            ...
            if not inserted:  ## No insertion occured : skip to next phase
                ...
                ## At root node : stay at root node and decrement active length and increment active edge
                    ...
                    if self.aLength > 0:  ## only have to if aLength> 0
                        self.aEdge = self.aNode.edges[self.text[j - self.remainder + 1]]
